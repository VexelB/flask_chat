...
def index():
	...
	if request.method == "POST":
		...
		if user and check_password_hash(user.password, request.form['psw']):
			login_user(user)
			if user.is_admin:
				flash('Вы успешно зашли в запись администратора', 'success')
				return redirect(url_for('admin'))
			else:
				return redirect(url_for('main_page'))
		else:
			flash('Такого пользователя нет или вы неверно ввели пароль!', category='error')
...
def admin():
	if current_user.is_admin and current_user.is_authenticated:
		...
		flash('Простите но вы не являетесь администратором!', 'error')
		return redirect(url_for('index'))
...
def edit_message():
	try:
		msg = Message.query.filter_by(id=int(request.form['msg_id'])).first()
...
def main_page():
	if current_user.is_admin:
		return redirect(url_for('admin'))
	return render_template('index.html', appname=APP_NAME, messages=messages)
...
def register():
	if request.method == "POST":
		if request.form['psw'] == request.form['psw2']:
			user = User(request.form['name'], generate_password_hash(request.form['psw']))
			db.session.add(user)
			db.session.commit()
			flash(f'Пользователь {request.form["name"]} успешно зарегистрирован!', category='success')
			return redirect(url_for('index'))
		else:
